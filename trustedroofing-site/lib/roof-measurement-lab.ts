...
export function components(mask:Raster,center:{latitude:number;longitude:number}){const v=mask.values[0],active=v.map(x=>Number.isFinite(x)&&x>0),seen=new Uint8Array(active.length),out:{pixels:number[];distancePx:number}[]=[],centerPixel=mask.wgs84ToPixel(center.longitude,center.latitude),cx=centerPixel.col,cy=centerPixel.row;for(let s=0;s<active.length;s++){if(!active[s]||seen[s])continue;const q=[s],pixels:number[]=[];seen[s]=1;let d=Infinity;for(let h=0;h<q.length;h++){const p=q[h],x=p%mask.width,y=~~(p/mask.width);pixels.push(p);d=Math.min(d,Math.hypot(x-cx,y-cy));for(const n of[p-1,p+1,p-mask.width,p+mask.width])if(n>=0&&n<active.length&&active[n]&&!seen[n]&&Math.abs(n%mask.width-x)<=1){seen[n]=1;q.push(n)}}out.push({pixels,distancePx:d})}return out.sort((a,b)=>a.distancePx-b.distancePx)}
