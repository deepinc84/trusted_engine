...
export type AcceptanceInput = { selectedTier?: string; selectedSoftScopeIds?: string[]; selectedVinylScopeIds?:string[]; scopeColours?: Record<string, string>; colour?: string; signerLegalName: string; signerEmail: string; signatureType: "typed" | "drawn"; signatureData: string; authorityAccepted: boolean; termsAccepted: boolean };
...
export function acceptanceIntegrity(snapshot: ProposalDraft, totals: ReturnType<typeof validateAcceptance>, input: AcceptanceInput, acceptedAt: string) {
  return createHash("sha256").update(JSON.stringify({ proposalNumber: snapshot.proposalNumber, revision: snapshot.revisionNumber, option: totals.option, selectedSoftScopes: totals.selectedSoftScopes,selectedVinylScopes:totals.selectedVinylScopes,declinedVinylScopes:totals.declinedVinylScopes, declinedSoftScopes: totals.declinedSoftScopes, scopeColours: totals.scopeColours, colour: input.colour, signer: input.signerLegalName, total: totals.total, acceptedAt })).digest("hex");
}
