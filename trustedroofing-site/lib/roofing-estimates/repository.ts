...
export type EstimateDraft = { id?: string; customer: { id?: string; firstName: string; lastName: string; email: string; phone: string }; property: { id?: string; addressLine1: string; addressLine2: string; city: string; province: string; postalCode: string }; measurements: RoofingMeasurements; systems: RoofingSystem[]; options?: CalculatedOption[]; updatedAt?: string };
...
export function newDraft(systems: RoofingSystem[] = DEFAULT_ROOFING_SYSTEMS): EstimateDraft { return { customer: { firstName: "", lastName: "", email: "", phone: "" }, property: { addressLine1: "", addressLine2: "", city: "Calgary", province: "AB", postalCode: "" }, measurements: { ...blankMeasurements }, systems: structuredClone(systems) }; }
...
function validate(draft: EstimateDraft) {
  ...
  if (draft.measurements.roofAreaSqft <= 0 && draft.measurements.squares <= 0) throw new Error("Roof area or squares is required.");
  if (draft.systems.length !== 3) throw new Error("Good, Better and Best system snapshots are required.");
}
...
export async function getEstimateDraft(id: string): Promise<EstimateDraft | null> {
  ...
  const { data: estimate, error } = await db.from("estimates").select("id,updated_at,customer_id,property_id,estimate_customers(*),estimate_properties(*),roofing_measurements(*),estimate_system_snapshots(tier,configuration),estimate_option_results(tier,breakdown,calculated_price,final_price)").eq("id", id).single();
...
export async function saveEstimateDraft(input: EstimateDraft, actor: string): Promise<EstimateDraft> {
  ...
  let id = input.id;
  if (id) { const { error } = await db.from("estimates").update({ customer_id: customerId, property_id: propertyId, updated_by: actor, updated_at: new Date().toISOString() }).eq("id", id); if (error) throw error; }
  else { const { data, error } = await db.from("estimates").insert({ customer_id: customerId, property_id: propertyId, created_by: actor, updated_by: actor }).select("id").single(); if (error) throw error; id = data.id; }
