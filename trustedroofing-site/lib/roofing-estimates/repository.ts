...
export function newDraft(systems: RoofingSystem[] = DEFAULT_ROOFING_SYSTEMS): EstimateDraft { return { estimateMode:"standard",specification:blankSpecificationEstimate(),scopeMode:"roofing",selectedScopes:["roofing"],softMetalScopes:blankSoftMetalScopes(),vinylScope:blankVinylScope(),hardieScope:blankHardieScope(), customer: { firstName: "", lastName: "", email: "", phone: "" }, property: { addressLine1: "", addressLine2: "", city: "Calgary", province: "AB", postalCode: "" }, measurements: { ...blankMeasurements }, systems: structuredClone(systems) }; }
...
function validate(draft: EstimateDraft) {
  ...
  if(draft.estimateMode==="specification"&&!draft.specification)throw new Error("Specification workspace data is required.");
}
...
export async function getEstimateDraft(id: string): Promise<EstimateDraft | null> {
  ...
  const { data: estimate, error } = await db.from("estimates").select("id,updated_at,scope,selected_scopes,estimate_mode,customer_id,property_id,estimate_customers(*),estimate_properties(*),roofing_measurements(*),estimate_system_snapshots(tier,configuration),estimate_option_results(tier,breakdown,calculated_price,final_price)").eq("id", id).single();
...
export async function saveEstimateDraft(input: EstimateDraft, actor: string): Promise<EstimateDraft> {
  ...
  let id = input.id;
  if (id) { const { error } = await db.from("estimates").update({ customer_id: customerId, property_id: propertyId, scope:selectedScopes.length===1?selectedScopes[0]:"multi_trade",selected_scopes:selectedScopes,estimate_mode:input.estimateMode??"standard", updated_by: actor, updated_at: new Date().toISOString() }).eq("id", id); if (error) throw error; }
  else { const { data, error } = await db.from("estimates").insert({ customer_id: customerId, property_id: propertyId, scope:selectedScopes.length===1?selectedScopes[0]:"multi_trade",selected_scopes:selectedScopes,estimate_mode:input.estimateMode??"standard", created_by: actor, updated_by: actor }).select("id").single(); if (error) throw error; id = data.id; }
  if(selectedScopes.includes("soft_metals"))await saveSoftMetalScopes(id as string,softMetalScopes,actor);if(selectedScopes.includes("vinyl_siding"))await saveVinylScope(id as string,vinylScope,actor);if(selectedScopes.includes("hardie_siding"))await saveHardieScope(id as string,hardieScope,actor);if(input.estimateMode==="specification")await saveSpecificationEstimate(id as string,"specification",specification,actor);
