...
export type EstimateDraft = { id?: string; scopeMode?:"roofing"|"soft_metals"|"combined"|"multi_trade"; selectedScopes?:EstimateScope[]; softMetalScopes?:SoftMetalScope[]; vinylScope?:VinylScope; customer: { id?: string; firstName: string; lastName: string; email: string; phone: string }; property: { id?: string; addressLine1: string; addressLine2: string; city: string; province: string; postalCode: string }; measurements: RoofingMeasurements; systems: RoofingSystem[]; options?: CalculatedOption[]; updatedAt?: string };
...
export async function saveEstimateDraft(input: EstimateDraft, actor: string): Promise<EstimateDraft> {
  ...
  if(selectedScopes.includes("soft_metals"))await saveSoftMetalScopes(id as string,softMetalScopes,actor);if(selectedScopes.includes("vinyl_siding"))await saveVinylScope(id as string,vinylScope,actor);
