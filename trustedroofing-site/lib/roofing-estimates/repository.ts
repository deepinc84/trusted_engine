...
export function newDraft(): EstimateDraft { return { customer: { firstName: "", lastName: "", email: "", phone: "" }, property: { addressLine1: "", addressLine2: "", city: "Calgary", province: "AB", postalCode: "" }, measurements: { ...blankMeasurements }, systems: structuredClone(DEFAULT_ROOFING_SYSTEMS) }; }
...
export async function saveEstimateDraft(input: EstimateDraft, actor: string): Promise<EstimateDraft> {
  validate(input); const options = input.systems.map((system) => calculateSystem(input.measurements, system));
  const db = getServiceClient();
