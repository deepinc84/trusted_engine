...
export async function POST(request: Request) {
  try {
    ...
    if (includeSolarPage) {
      ...
      const productionY = verdictY - 82;
      drawCard(page4, MARGIN, productionY, CONTENT_WIDTH, 72, COLORS.cardBg);
      drawText(page4, "Estimated annual production range", MARGIN + 12, productionY + 54, 10.8, COLORS.textDark, "F2");
      const valueCardW = (CONTENT_WIDTH - 20) / 3;
      const productionValues = [
        { label: "LOW", value: solarSnapshot.estimatedProductionLowKwh },
        { label: "LIKELY", value: solarSnapshot.estimatedProductionLikelyKwh },
        { label: "HIGH", value: solarSnapshot.estimatedProductionHighKwh }
      ];
      productionValues.forEach((item, index) => {
        const x = MARGIN + 8 + index * (valueCardW + 2);
        drawRoundedBox(page4, x, productionY + 18, valueCardW, 26, COLORS.cardSoft, COLORS.border);
        drawTextCentered(page4, item.label, x + valueCardW / 2, productionY + 35.5, 6.6, COLORS.textSub, "F1");
        drawTextCentered(page4, `${item.value ? Math.round(item.value).toLocaleString() : "n/a"} kWh`, x + valueCardW / 2, productionY + 23.5, 8.3, COLORS.textDark, "F2");
      });
      drawText(page4, "Final production depends on layout, usable roof planes, system design, and actual household usage.", MARGIN + 12, productionY + 8, 7.2, COLORS.textMid);

      const meaningY = productionY - 52;
      ...
      const whyY = structureY - 58;
      drawCard(page4, MARGIN, whyY, CONTENT_WIDTH, 50, COLORS.cardSoft);
      ...
      const nextY = whyY - 54;
      drawCard(page4, MARGIN, nextY, CONTENT_WIDTH, 46, COLORS.cardBg);
      ...
      drawText(page4, "Program information may vary by municipality and provider.", MARGIN + 12, 80, 6.6, COLORS.textSub);
