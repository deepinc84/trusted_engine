...
export default async function QuoteActivitySummary() {
  ...
  return (
    <section className="ui-card quote-support-card" aria-labelledby="calgary-activity-title">
      ...
      <h2 id="calgary-activity-title">Recent Estimate Activity Across Calgary</h2>
      <p>
      ...
      <p>
        For a scope-specific baseline, start with the <Link href="/online-estimate">instant roof quote tool</Link>
        {" "}or browse <Link href="/quotes">recent Calgary quote examples</Link> before comparing roofing, siding, and eavestrough options.
      </p>
      <h3 style={{ marginBottom: 8 }}>Recent Estimator Activity by Quadrant</h3>
      <ul style={{ marginTop: 0 }}>
        {quadrantRows.map((row) => (
          <li key={row.label}>
            <strong>{row.label}:</strong> {row.value > 0 ? `${row.value} recent estimate examples` : "Active seasonal pricing shifts; generate an address-based estimate for current range."}
          </li>
        ))}
      </ul>
